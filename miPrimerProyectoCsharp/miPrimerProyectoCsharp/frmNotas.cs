...
namespace miPrimerProyectoCsharp
{
    public partial class frmNotas : Form
    {
        ...
        private void actualizarDs()
        {
            ...
            objConexion.objAdaptador = new SqlDataAdapter("SELECT alumnos.nombre, dnotas.IdDetalle, dnotas.idNota, dnotas.idMateria, dnotas.lab1, dnotas.lab2, dnotas.parcial, " +
                "(dnotas.lab1 * 0.3 + dnotas.lab2 * 0.3 + dnotas.parcial * 0.4) AS nf " +
                "FROM dnotas INNER JOIN notas ON(dnotas.idNota=dnotas.idNota) INNER JOIN alumnos ON(alumnos.idAlumno=notas.idAlumno) " +
                "WHERE notas.idPeriodo=" + idPeriodo + " AND dnotas.idMateria = " + idMateria, objConexion.objConexion); //Seleccionar todo de una tabla
            objConexion.objAdaptador.Fill(objNotas, "notasAlumnos"); //Tomando los datos de la BD y llenando DataSet
        }
